...
2. Call `cloud_polygraph_delegate` with:
...
3. Collect the child agent's response. If the delegation fails or times out, log the error and skip that repo.

4. Display failure summary for each repo:
   ```
   Repository: frontend
   CI Pipeline: <cipeUrl>
   Failed Tasks (2):
