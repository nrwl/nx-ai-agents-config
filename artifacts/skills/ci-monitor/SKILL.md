...
name: ci-monitor
description: |
  Monitor Nx Cloud CI pipeline and handle self-healing fixes. USE WHEN user asks to monitor CI, watch pipeline, check CI status, wait for green, check Nx Cloud, track build, or after pushing changes that need CI verification. Auto-applies verified fixes.
allowed-tools:
  - Bash
  - Read
  - Write
...
**The `mcp__nx-mcp__ci_information` MCP tool is the ONLY authoritative source for CI status.**

- Do NOT use `gh pr checks`, `gh run view`, or GitHub Actions UI as fallback
- Do NOT use WebFetch to scrape Nx Cloud pages
- If the MCP tool fails, retry with backoff - do not fall back to other methods
- The MCP tool provides structured self-healing data (fix content, verification status, shortLink) that gh CLI cannot
...
[ci-monitor] Cycle 1 | Elapsed: 5m | CIPE: FAILED | Self-Healing: COMPLETED
[ci-monitor] Fix available! Verification: COMPLETED, Confidence: 85%
[ci-monitor] Applying fix via MCP...
[ci-monitor] Fix applied in CI. Waiting for new CIPE...

[ci-monitor] Spawning subagent to poll CI status...
[ci-monitor] Cycle 2 | Elapsed: 12m | CIPE: SUCCEEDED
