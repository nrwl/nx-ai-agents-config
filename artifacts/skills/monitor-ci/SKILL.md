...
| `fix_throttled`     | Self-healing throttled (too many unapplied fixes). Attempt local fix based on `taskOutputSummary`. If successful → commit, push, loop. If not → exit with failure + guidance. |
